...
| **Sentry stores city-level location and a timezone, and the client cannot strip either** | `user.geo` — *VN, Tan An, Vietnam* — is derived from the connecting IP **at ingest**, so no `beforeSend` reaches it; it goes off in project settings or not at all. `contexts.culture` carries locale and timezone and *is* client-side, so that half is a code fix. Neither is a scrubber failure, and both sit outside what *stripped at the client* can promise. `docs/identity-and-access.md` has the distinction | **open** · settings change is Maciej's; the `culture` drop is Quiz Maker's |
