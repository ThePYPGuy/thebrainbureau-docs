...
**Source maps upload; the join was severed and is repaired.** `87a737e` stops the
scrubber redacting Sentry's `debug_id`. A production payload now carries an
intact id matching an uploaded map — both sides checked this time. **Nobody has
yet seen a mapped frame**; that needs the Sentry UI. *[verify]*
