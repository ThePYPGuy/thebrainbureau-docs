...
| **`e2e` went from 8 of 8 to 5, and the difference is live** | `e2e-class-join` fails three checks on a clean `main` — **`npm run e2e`** reproduces it, and I ran it independently of the session that found it. A child on a class-code session with no mission is served `/terminal` **200, no location**, where `app/terminal/page.tsx:13` plainly redirects on `!session.deploymentId` — so the branch did not fire and the session HAD a deployment. Suspect `7e4acee`, the stale-cookie and proxy change, which `git merge-base --is-ancestor 7e4acee origin/main` confirms is inside the 50 already pushed. **Not necessarily the code**: an auth change that is correct would break a test whose setup no longer produces the state it assumes, and the two are indistinguishable from outside — establish which before changing either | **open** — *(WD)*, and until it is, **nobody knows what a child sees in that state** |
