...
| **Signal Check's reveal is off by one, and it is live** | `revealFor` does `this.questions[position]` — a 0-based array indexed by a 1-based position — so the board shows the **next** question's answer, explanation and option labels, and scores the class against the wrong key. A correct answer is marked wrong. The header reads *QUESTION 2 OF 22* for the first question served. Found while staging a screenshot, isolated to one player and one answer, and verified against `session_questions` | **open, urgent** · Quiz Maker. **No class should play Signal Check until this lands** |
