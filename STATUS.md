...
| **Adding a column to `activities` is FOUR edits and the fourth fails late** | The migration, the payload in `import-activity.ts`, the `insert into activities (...)` list in `20260831000016`, **and its separate `on conflict do update set` list** — which the repo's own comment folds into “the SQL function”. **Miss the fourth and the column populates on first import, then silently stops updating**; `content_hash` is in the update list, so the hash still moves and **`deploy:check` reports a match** while the value stays frozen | **`npm run test:columns` exists and has never been proven against THIS case.** Omit list 4 deliberately, edit the value, re-import: if it passes, it is not testing the round trip it claims to |
