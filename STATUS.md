...
| **A redirect now reports 200, and one script reads that as success** | `e2e-class-join` went red on three checks and **the code was correct throughout** — a `loading.tsx` above the route makes Next stream the redirect inside a 200 document. Ruled out by decoding the signed cookie: `deploymentId: null`, so the guard fired every time. Fixed at `ff6bc0a` by asserting **where a request lands** rather than its status, which also catches a 200 that served a mission — something the old check would have passed. **The remaining exposure is `scripts/print-proof.ts:68`**, which treats `status !== 200` as failure and would now parse a redirect stub as a terminal page: a false PASS, not a false alarm. `scripts/check-offsite.ts:137` already guards this by checking where it landed | **fixed** in `e2e`; **print-proof open** *(WD)* |
