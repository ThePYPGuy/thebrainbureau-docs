...
| **Adding a column to `activities` is FOUR edits, and `test:columns` did not catch the fourth** | The migration, the payload in `import-activity.ts`, the `insert into activities (...)` list in `20260831000016`, **and its separate `on conflict do update set` list**. Miss the fourth and the column populates on first import, then silently stops updating. **MEASURED, and the guard passed the bug:** with `structure` in three lists and out of the fourth, a file authored `open-board` re-imported into a row still reading `linear`, **`deploy:check` said “This database matches the repo” and `test:columns` passed all thirteen of its checks** — `20260902000036_activity_structure.sql:49-53`, measured before the migration was written | **third guard on this project to agree with its own bug.** `test:columns` now carries a fourth comparison and fails on that arrangement — **unverified by me: reported by the session that ran it** |
