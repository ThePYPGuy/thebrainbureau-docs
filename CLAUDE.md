...
**`LayoutProps` is generated by the build**, at `.next/types/routes.d.ts`. A
worktree that has never run `npm run build` or `next dev` has no `.next/types`
at all, so the type does not exist and `app/layout.tsx` fails to compile.

`new-worktree.sh` copies `.env.local` and hard-links `node_modules`, and its own
sanity check (`npm run test:columns`) passes without a build — **so the tree
looks complete and cannot typecheck.** Build once and it goes away.

**Why it is worth its lines: the error names a file somebody wrote.** It reads
as a defect in `layout.tsx`, and the first session to meet it reported it as a
pre-existing failure on `main`. `main` typechecks clean. **Before calling
anything pre-existing, check whether the tree has ever been built** — naming a
baseline COMMIT is the right instinct and the wrong baseline here.

### An optional secret is a secret present on some code paths

`lib/server/state.ts` serialises `tasks.config` to the browser **wholesale**,
stripping only `hint` and `_`-prefixed notes. That is safe only because the
answer was never in config: it lives in `task_answers`, its own table, and
migration `20260823000002` says why — **RLS filters ROWS, not columns**, so it
cannot hide an answer column from someone allowed to read the task.
`dataset_rows` / `dataset_row_keys` is the same split for the same reason.

So any config union with an answer inside it ships **every** answer to **every**
browser, through a serialiser behaving exactly as designed. The Lock Library
therefore carries two unions and **no `answer` field at all — not even an
optional one**, because the point of the split is that a lock handed to a
renderer has nowhere to put an answer.

**And types are not the whole guard.** A migration, a fix-up script or a
hand-written row can still put an answer-shaped key into config — which is
exactly how `_`-notes once reached a response despite the importer stripping
them. **Import-time stripping protects what arrives through the importer and
nothing else.** Assert on the read path too.
